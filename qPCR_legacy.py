...
if not cleaned_data.empty:
    ...
    if st.button('Perform ΔΔCt Analysis') and reference_genes and target_genes:
        # Filtering data for target genes
        ...
        # Step 2: Merge with target gene data and calculate dCp
        ...
        # Step 3: Calculate Ave, ddCp, and Fold Change
        grouped_data = merged_data.groupby([sample_id_column, 'Gene'])
        merged_data['Ave'] = grouped_data['dCp'].transform('mean')
        merged_data['ddCp'] = merged_data['dCp'] - merged_data['Ave']
        merged_data['Fold_Change'] = 2 ** (-merged_data['ddCp'])

        # Step 4: Calculate SD and SEM for Fold Change
        grouped_data = merged_data.groupby([sample_id_column, 'Gene'])
        ...
        st.write("DataFrame with Calculated Values:", merged_data)
        ...
        # Saving analysis data and t-test results in session state
        st.session_state['analysis_data'] = merged_data
